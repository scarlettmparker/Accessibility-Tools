...
class NeuralNet(nn.Module):
    def __init__(self, in_channels=3, use_auxiliary=True, num_classes=1000):
        ...
        # define auxiliary classifiers if required
        if use_auxiliary:
            self.auxiliary4a = Auxiliary(512, num_classes)
            self.auxiliary4d = Auxiliary(528, num_classes)
            
        # inception blocks for capturing multi-scale features
    ...
    def forward(self, x):
        ...
        # forward pass through the first conv layers & pooling        
        x = self.conv1(x)
        x = self.maxpool(x)
        x = self.conv2(x)
        x = self.maxpool(x)
...
class InceptionBlock(nn.Module):
    def __init__(self, in_channels, num_1x1, num_3x3_red, num_3x3, num_5x5_red, num_5x5, num_pool_proj):
        ...
        #3x3 convolutional block with reduction layer
        ...
        #5x5 convolutional block with reduction layer
...
class Auxiliary(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        # pooling and conv layers for auxiliary classifier
